...
#define HOST_VACCED(x) (Hosts[x].vacc_start_time+g_allParams.usVaccTimeToEfficacy<=ts)
#define HOST_VACCED_SWITCH(x) (Hosts[x].vacc_start_time >= g_allParams.usVaccTimeEfficacySwitch)
#define HOST_QUARANTINED(x) ((Hosts[x].quar_comply == 1) && (Hosts[x].quar_start_time + g_allParams.usHQuarantineHouseDuration > ts) && (Hosts[x].quar_start_time <= ts))
#define HOST_TO_BE_QUARANTINED(x) ((Hosts[x].quar_start_time + g_allParams.usHQuarantineHouseDuration > ts) && (Hosts[x].quar_comply < 2))
#define HOST_ISOLATED(x) ((Hosts[x].isolation_start_time + g_allParams.usCaseIsolationDelay <= ts) && (Hosts[x].isolation_start_time + g_allParams.usCaseIsolationDelay + g_allParams.usCaseIsolationDuration > ts))
